...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 4243
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 4243
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 14242
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 14242
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 4243
	%{_sbindir}/semanage port -d -p tcp 4243
	%{_sbindir}/semanage port -d -p tcp 14242
	%{_sbindir}/semanage port -d -p tcp 14242
	for selinuxvariant in %{selinux_variants}; do
